...
export async function runSimulation(req: Request, res: Response): Promise<void> {
    ...
    if (!params.type || !params.mode) {
        res.status(400).json({ error: 'Simulation type and mode are required' });
        return;
    }
    ...
    try {
        ...
        res.status(500).json({ error: error.message || 'Internal server error during simulation' });
    }
}
    ...
    res.json([
        {
            id: 'time_windows',
            name: 'Time Windows',
            description: 'Simulates trading only during specific high-performance time windows.',
        },
        {
            id: 'daily_tp',
            name: 'Daily Take Profit',
            description: 'Simulates stopping trading for the day once a profit threshold is reached.',
            params: [
                {
                    name: 'threshold',
                    label: 'TP Threshold ($)',
