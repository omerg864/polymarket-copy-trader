...
export async function getCandles(req: Request, res: Response): Promise<void> {
	...
	const fetchBatch = async (start: string | undefined, end: string | undefined) => {
	...
	try {
		...
		const targetEndTime = endTime ? parseInt(endTime as string, 10) : Date.now();
		const maxPages = 3;

		for (let i = 0; i < maxPages; i++) {
			const batch = await fetchBatch(currentStartTime, endTime as string | undefined);
