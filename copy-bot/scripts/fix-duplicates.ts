...
async function fixDuplicates() {
	...
	try {
		...
		for (const [id, group] of tradeGroups.entries()) {
			if (group.length > 1) {
				console.log(
					`⚠️  Found ${group.length} entries for trade ID ${id} (${group[0].title})`,
				);

				// Identify the FCT one (or just the second one if both are same)
				const fctIndex = group.findIndex(
					(t) =>
					...
					group.find(
						(t) =>
							...
							(t.status === 'closed_sell' &&
								group.some(
									(other) =>
										other.status === 'closed_tp' ||
										other.status === 'closed_sl',
								)),
					) || group[1]; // Fallback to second one

				console.log(
					`🗑️  Removing duplicate entry: Status=${toRemove.status}, PnL=$${toRemove.pnl.toFixed(2)}`,
				);
				...
				const stringToRemove = rawHistory.find((h) => {
					...
					return (
						parsed.id === toRemove.id &&
						parsed.status === toRemove.status &&
						parsed.pnl === toRemove.pnl
					);
				});
				...
				if (stringToRemove) {
					...
					const sellFee = toRemove.size * toRemove.exitPrice! * 0.0; // Fee is 0 for limit? No, wait.
					...
					revenueToSubtract +=
						(toRemove.exitPrice || 0) * toRemove.size;
		...
		if (duplicatesFound > 0) {
			...
			console.log(
				`📉 Adjusting balance by -$${totalToSubtract.toFixed(2)}...`,
			);

			await redis.set(
				balanceKey,
				(currentBalance - totalToSubtract).toString(),
			);
			...
			console.log(
				`✨ DONE! New Balance: $${(currentBalance - totalToSubtract).toFixed(2)}`,
			);
		} else {
