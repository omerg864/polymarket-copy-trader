...
export const BalanceGraph: React.FC<BalanceGraphProps> = ({
	...
	const chartData = useMemo(() => {
		...
		const firstTime = DateTime.fromMillis(balancePoints[0].time)
			.setZone(timezone)
			.startOf('hour');
		const lastTime = DateTime.fromMillis(
			balancePoints[balancePoints.length - 1].time,
		)
			.setZone(timezone)
			.endOf('hour');
		...
		while (runner <= lastTime) {
			...
			while (
				pointIdx < balancePoints.length &&
				balancePoints[pointIdx].time <= hourEnd
			) {
	...
	const currentProgress =
		chartData.length > 0
			? chartData[chartData.length - 1].value - initialBalance
			: 0;
	const progressPercent =
		initialBalance > 0 ? (currentProgress / initialBalance) * 100 : 0;
			...
			<CardHeader className="pb-2">
				<div className="flex justify-between items-center">
					...
					<div className="text-right">
						...
						>
							{formatBtcPrice(currentProgress)}
						</div>
						<div
							className={`text-[10px] font-bold uppercase tracking-widest ${isProfit ? 'text-emerald-500/70' : 'text-red-500/70'}`}
						>
