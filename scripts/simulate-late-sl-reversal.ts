...
async function main() {
	...
	for (const t of allTrades) {
		...
		if (
			t.status === 'closed_sl' &&
			closedAt &&
			endTime.diff(closedAt).as('seconds') < 4
		) {
			isAffected = true;
	...
	if (lateSlTrades.length === 0) {
		console.log(
			'No trades found that hit SL within 4 seconds of market close.',
		);
	} else {
	...
	console.log(
		`TOTAL PNL DELTA:        $${(totalSimulatedPnl - totalOriginalPnl).toFixed(2)}`,
	);
	console.log('='.repeat(100));
