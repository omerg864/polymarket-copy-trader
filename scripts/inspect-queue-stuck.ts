...
async function main() {
	...
	const completionQueue = new Queue('trade-completion', {
		connection: redis,
	});
	...
	for (const id of stuckIds) {
		...
		if (job) {
			...
			console.log(
				`Job ${id}: state=${state}, attempts=${job.attemptsMade}, failedReason=${job.failedReason}`,
			);
		} else {
	...
	for (const id of stuckIds) {
		...
		if (job) {
			const state = await job.getState();
			console.log(
				`Job ${jobId}: state=${state}, attempts=${job.attemptsMade}, failedReason=${job.failedReason}`,
			);
		} else {
