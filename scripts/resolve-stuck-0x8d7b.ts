...
const tradeId =
	'0x8d7b767b9d0b6b9f72121395c9e1eb04a614c1dad3a5c657c73d683841db1145';
...
async function main() {
	...
	if (!rawTrade) {
		console.error(
			`❌ Trade ${tradeId} not found in Redis! Check prefix and ID.`,
		);
		process.exit(1);
	}
	...
	if (trade.status !== 'open') {
		console.warn(
			`⚠️ Trade status is '${trade.status}', not 'open'. Are you sure?`,
		);
	}
	...
	const doc = await TradeModel.findOneAndUpdate({ tradeId }, mongoUpdate, {
		new: true,
	});
	if (doc) {
